...
// vlist_triangles
  if(g_mesh_model == kCube && g_mesh_type == kVlistTriangles)
  {
  ...
  if(g_mesh_model == kAvocado && g_mesh_type == kVlistTriangles)
  {
  ...
  if(g_mesh_model == kDonut && g_mesh_type == kVlistTriangles)
  {
...
void render_object()
{
  ...
  // ############### index_buffer ################# 추가
  // 앞으로 언급하는 배열 버퍼(GL_ELEMENT_ARRAY_BUFFER)는 index_buffer로 지정
